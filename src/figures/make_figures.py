...
def make_quantile_plots(
    ...
    for model in models_list:
        ...
        plt.tight_layout()
        plt.savefig(fig_dir / f"{model}_quantile_plot_{country}_{horizon_in_quarters}q_{target_dict[target_idx]}.png", bbox_inches='tight', dpi=300)
        plt.close()
...
def make_mean_plots(
    ...
    for model in models_list:
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(fig_dir / f"{model}_mean_plot_{country}_{horizon_in_quarters}q_{target_dict[target_idx]}.png", dpi=300)
        plt.close(fig)
